...
void Actor::Update( float deltaSeconds )
{
	...
	{
		// Wander randomly for now
		if ( m_wanderDirectionChangeCooldown <= 0.f )
		{
			m_orientationDegrees = g_game->m_rng->RollRandomFloatInRange( 0.f, 360.f );
			m_wanderDirectionChangeCooldown = ACTOR_WANDER_DIRECTION_CHANGE_COOLDOWN_SECONDS;
		}
		else
		{
		...
		m_velocity += m_actorDef->m_walkSpeed * GetForwardVector();
	}
