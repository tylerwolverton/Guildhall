...
void MapGenStep_RoomsAndPaths::BuildRooms( Map& map, std::vector<AABB2>& existingRoomBounds )
{
	...
	AABB2 mapBounds( 0, 0, map.m_width, map.m_height );
...
const AABB2 MapGenStep_RoomsAndPaths::GetBoundsForNewRoom( int roomWidth, int roomHeight, const AABB2& mapBounds, bool canOverlap, const std::vector<AABB2>& existingRoomBounds )
{
	...
	{
		IntVec2 roomMins( g_game->m_rng->RollRandomIntLessThan( (int)mapBounds.maxs.x ),
						  g_game->m_rng->RollRandomIntLessThan( (int)mapBounds.maxs.y ) );

		IntVec2 roomMaxs( roomMins );
...
void MapGenStep_RoomsAndPaths::ConnectRoomsWithPath( Map& map, const AABB2& room1, const AABB2& room2 )
{
	...
	for ( int tileNum = 0; tileNum < totalLength; ++tileNum )
	{
		...
		{
			AddVerticalTile( map, curPos, pathStep.y, remainingPathLength );
			continue;
		}

		if ( remainingPathLength.y == 0 )
		{
			AddHorizontalTile( map, curPos, pathStep.x, remainingPathLength );
			continue;
		}
...
void MapGenStep_RoomsAndPaths::PopulatePathParams( const AABB2& room1, const AABB2& room2, Vec2& out_pathStart, Vec2& out_pathEnd, Vec2& out_pathStep )
{
	...
	//// Room 1 -- Room 2
	//if ( room1.maxs.x < room2.mins.x )
	//{
	//	out_pathStart.x = room1.maxs.x;
	//	out_pathEnd.x = room2.mins.x;
	//	//pathStart.x = room1.mins.x + ( room1.GetWidth() * .5f );
	//	//pathEnd.x	  = room2.mins.x - ( room2.GetWidth() * .5f );
	//}
	//// Room 2 -- Room 1
	//else
	//{
	//	out_pathStart.x = room1.mins.x;
	//	out_pathEnd.x = room2.maxs.x;
	//	/*pathStart.x = room1.mins.x - ( room1.GetWidth() * .5f );
	//	pathEnd.x	= room2.maxs.x + ( room2.GetWidth() * .5f );*/
	//	out_pathStep.x *= -1.f;
	//}
	...
	////   |
	//// Room 2
	//if ( room1.maxs.y < room2.mins.y )
	//{
	//	out_pathStart.y = room1.mins.y;
	//	out_pathEnd.y = room2.maxs.y;
	//	/*pathStart.y = room1.mins.y + ( room1.GetHeight() * .5f );
	//	pathEnd.y	= room2.maxs.y - ( room2.GetHeight() * .5f );*/
	//}
	//// Room 2
	////   |
	//// Room 1
	//else
	//{
	//	out_pathStart.y = room1.maxs.y;
	//	out_pathEnd.y = room2.mins.y;
	//	/*pathStart.y = room1.maxs.y - ( room1.GetHeight() * .5f );
	//	pathEnd.y	= room2.mins.y + ( room2.GetHeight() * .5f );*/
	//	out_pathStep.y *= -1.f;
	//}
}
...
//-----------------------------------------------------------------------------------------------
void MapGenStep_RoomsAndPaths::AddHorizontalTile( Map& map, Vec2& curPos, float nextTileStep, IntVec2& remainingPathLength )
{
...
//-----------------------------------------------------------------------------------------------
void MapGenStep_RoomsAndPaths::AddVerticalTile( Map& map, Vec2& curPos, float nextTileStep, IntVec2& remainingPathLength )
{
