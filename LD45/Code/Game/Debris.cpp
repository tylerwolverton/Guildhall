	...
	, m_color( color )
{
	...
	m_velocity = -velocity;
	m_angularVelocity = m_game->m_randNumGen->GetRandomFloatInRange( -300.f, 300.f );
}
...
void Debris::Render() const
{
	...
	g_theRenderer->DrawVertexArray( NUM_VERTEXES, copyOfLocalVerts );
}
...
void Debris::PopulateVertexes()
{
	...
	for ( int vertexIndex = 0; vertexIndex < NUM_DEBRIS_VERTS; )
	{
		...
		if ( vertexIndex == 0 )
		{
			float radius = m_game->m_randNumGen->GetRandomFloatInRange( m_physicsRadius, m_cosmeticRadius );
