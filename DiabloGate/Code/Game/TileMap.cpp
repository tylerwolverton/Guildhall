...
void TileMap::UpdateMesh()
{
	...
	for ( int tileIdx = 0; tileIdx < (int)m_tiles.size(); ++tileIdx )
	{
		...
		m_mesh.push_back( Vertex_PCU( bottomLeft, materialTypeDef->GetSpriteTint(), uvsAtMins ) );
		m_mesh.push_back( Vertex_PCU( bottomRight, materialTypeDef->GetSpriteTint(), Vec2( uvsAtMaxs.x, uvsAtMins.y ) ) );
		m_mesh.push_back( Vertex_PCU( topRight, materialTypeDef->GetSpriteTint(), uvsAtMaxs ) );

		m_mesh.push_back( Vertex_PCU( bottomLeft, materialTypeDef->GetSpriteTint(), uvsAtMins ) );
		m_mesh.push_back( Vertex_PCU( topRight, materialTypeDef->GetSpriteTint(), uvsAtMaxs ) );
		m_mesh.push_back( Vertex_PCU( topLeft, materialTypeDef->GetSpriteTint(), Vec2( uvsAtMins.x, uvsAtMaxs.y ) ) );
	}
}
...
{
	if ( g_game->GetCameraMode() == eCameraMode::LOCKED_TO_PLAYER )
	{
...
//-----------------------------------------------------------------------------------------------
void TileMap::KeepCameraInBounds() const
{
	Vec2 centerOfScreen = g_game->GetWorldCameraFocalPoint2D();

	Vec2 halfWindowSize( WINDOW_WIDTH * .5f, WINDOW_HEIGHT * .5f );
	AABB2 cameraBounds( centerOfScreen - halfWindowSize, centerOfScreen + halfWindowSize );
