...
{
	PopulateGameConfig();
	m_isQuitting = CheckAndRunCommandlets();
}
...
void App::Startup()
{
	std::string windowTitle = g_gameConfigBlackboard.GetValue( "windowTitle", "Model Viewer" );
...
//-----------------------------------------------------------------------------------------------
bool App::CheckAndRunCommandlets()
{
	std::string commandLineArgs = g_gameConfigBlackboard.GetValue( "commandLineArgs", "" );
	Strings args = SplitStringOnDelimiter( commandLineArgs, ' ' );

	if ( args.size() == 2
		 && args[0] == "cookObj" )
	{
		std::string const& modelPath = args[1];
		...
		AppendVertsAndIndicesForObjMeshFromFile( vertices, indices, Stringf( "Data/%s.obj", modelPath.c_str() ), importOptions );
		CPUMesh mesh( vertices, indices );
		SaveMeshAsTWSMFile( mesh, Stringf( "Data/%s.twsm", modelPath.c_str() ) );
