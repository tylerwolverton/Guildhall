...
cbuffer debug_lighting_constants : register( b10 )
{
	float    DIFFUSE_EFFECT;
	float    SPECULAR_EFFECT;
	float    EMISSIVE_EFFECT;
};
...
Texture2D <float4> tSpecGlossEmit   : register( t2 );   // combined specular, glossy, and emissive map of the surface
