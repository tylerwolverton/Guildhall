...
fragment_output_t FragmentFunction( v2f_t input )
{
	...
	float4 specGlossEmit_color = tSpecGlossEmit.Sample( sSampler, input.uv );

	/*fragment_output_t outputTemp;
	outputTemp.color = specGlossEmit_color;
	outputTemp.bloom = specGlossEmit_color;
	return outputTemp;*/
	...
	lit_color_t final_color = CalculateDot3Light( input.world_position, world_normal, surface_color, specGlossEmit_color );
